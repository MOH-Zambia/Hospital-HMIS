...
from app.models import ICD10, DHIS2_ICD10, OPDEvent, IPDEvent
...
def autocomplete_icd10_diagnosis(request):
    if request.is_ajax():
        ...
        search_qs = DHIS2_ICD10.objects.filter(description__contains=q)
        results = []
...
def export_opd_events(request):
    ...
    for opd_event in opd_events:
         writer.writerow([
            ...
            opd_event.location,
            opd_event.gender,
            opd_event.diagnosis.split(" ", 1)[0],
            opd_event.secondary_diagnosis.split(" ", 1)[0],
            opd_event.other_diagnosis.split(" ", 1)[0],
...
def export_ipd_events(request):
    ...
    for ipd_event in ipd_events:
         writer.writerow([
            ...
            ipd_event.diagnosis.split(" ", 1)[0],
            ipd_event.secondary_diagnosis.split(" ", 1)[0],
            ipd_event.other_diagnosis.split(" ", 1)[0],
